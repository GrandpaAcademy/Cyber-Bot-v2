...
function botLog() {
  ...
  const _ = require(path.resolve('config.json'));
  ...
  let title = text || '\u0043\u0079\u0062\u0065\u0072\u002d\u0042\u006f\u0074\u002d\u0076\u0032';
  const lines = ['', '', '', '', '', ''];
  for (let i = 0; i < title.length; i++) {
    const char = title[i].toLowerCase();
    const mapping = asciiMappings[char] || asciiMappings[' '];
    lines[0] += mapping.upper || '    ';
    lines[1] += mapping.lower || '    ';
    lines[2] += mapping.middle || '    ';
    lines[3] += mapping.bottom1 || '    ';
    lines[4] += mapping.bottom2 || '    ';
    lines[5] += mapping.final || '    ';
  }
  return lines.join('\n');
}
